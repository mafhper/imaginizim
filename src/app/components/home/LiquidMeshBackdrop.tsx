...
export function LiquidMeshBackdrop() {
  ...
  useEffect(() => {
    ...
    const updateColors = (theme: string) => {
      ...
      // Hyper-vibrant "Festival" explosion of colors
      if (isDark) {
  ...
  return (
    <div ref={rootRef} className="liquid-mesh fixed inset-0 z-[-1]" aria-hidden="true">
      <div className="liquid-mesh__fallback fixed inset-0 bg-background" />
      <canvas ref={canvasRef} className="liquid-mesh__canvas h-full w-full object-cover" />
    </div>
  );
}
