...
const FRAGMENT_SHADER_SOURCE = `
  ...
  void main() {
    ...
    float tealRibbon = smoothstep(0.55, 1.0, sin((p.x * 1.6 + p.y * 0.4) * 2.2 + h * 1.4 + uTime * 0.12));
    float violetRibbon = smoothstep(0.7, 1.0, cos((p.x * 1.1 - p.y * 0.65) * 2.5 - h * 1.2 + uTime * 0.16));
    float blueField = smoothstep(0.56, 1.0, cos((p.x * 0.55 + p.y * 0.45) * 1.9 + h * 1.2 - uTime * 0.09));

    vec3 signal = mix(uColorB, uColorE, blueField * 0.36);
    signal = mix(signal, uColorD, violetRibbon * 0.1);

    vec3 distant = mix(uColorA, signal, bands * 0.48);
    distant = mix(distant, uColorC, (sheen * 0.11 + tealRibbon * 0.12) * horizon);
    vec3 base = mix(distant * 0.62, distant, sky * 0.48 + ground * 0.28);
    base += (specA * signal + specB * uColorC) * (0.04 + uChrome * 0.16) * horizon;
    ...
    color += signal * (0.055 + bands * 0.08) + uColorC * 0.018;
...
function createProgram(gl: WebGLRenderingContext): LiquidMeshProgram {
...
export function LiquidMeshBackdrop() {
  ...
  useEffect(() => {
    ...
    if (reduceMotion?.matches || window.navigator.userAgent.toLowerCase().includes('jsdom')) {
      canvas.hidden = true;
      return;
    }
    ...
    const resize = () => {
      if (!gl) return;
      const ratio = Math.min(window.devicePixelRatio || 1, LIQUID_MESH_PRESET.resolution);
    ...
    const render = (now: number) => {
      ...
      gl.uniform2f(meshProgram.uniforms.uResolution, canvas.width, canvas.height);
  ...
  return (
    <div ref={rootRef} className="liquid-mesh" aria-hidden="true">
      <div className="liquid-mesh__fallback" />
      <canvas ref={canvasRef} className="liquid-mesh__canvas" />
    </div>
  );
}
